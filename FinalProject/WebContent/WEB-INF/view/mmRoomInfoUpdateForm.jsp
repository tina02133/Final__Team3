...
<link
	href="https://fonts.googleapis.com/css2?family=Hi+Melody&family=Nanum+Gothic+Coding&display=swap"
	rel="stylesheet">
...
<body>
	...
	<!-- form 시작  -->
	<form action="memaUpdate.kkini" method="post">
		<!--개설폼 div  -->
		<div class="openBox">
			<div class="scoreBtn">
				<span class="checkScore" style="background-color: orange;">내
					점수 확인하기!</span>
				<div class="ScoreText"
					style="background-color: #444444; opacity: 0.9;">
					출석 점수 : ${getScore.attendScore}<br> 신뢰 점수 :
					${getScore.mannerScore}<br> 매너 점수 : ${getScore.biasScore}<br>
					<p class="alertText">※개설자님의 출석,신뢰,매너 점수는 지원자에게 전체공개됩니다</p>
				</div>
			</div>
			<div class="content">
				<p class="infoText">
					<span class="mm">메뉴메이트</span>수정
				</p>

				<br>
				<div>
					<span class="red">※</span> 방문예정 일시<br> <input type="text"
						class="form-control-lg form-control-date" style="width: 500px;"
						id="datetime" name="mmVisitDate" placeholder="클릭하여 선택해주세요."
						value="${roomInfo.visitDate }" readonly="readonly" required>
				</div>
				<br>
				...
				<div class="map_wrap">
					...
					<div id="menu_wrap" class="bg_white">
						<div class="option">
							<div>
								키워드 : <input type="text" value="${roomInfo.restLocation }"
				...
				<!--카카오맵 API APP KEY  -->
				<script type="text/javascript"
					src="//dapi.kakao.com/v2/maps/sdk.js?appkey=73ce8ac5774ecbf47e6474dc618e62f2&libraries=services"></script>
				<!--카카오맵 자바스크립트  -->
				<script type="text/javascript">
			// 마커를 담을 배열입니다
			var markers = [];
			 ...
			 function clickAction()
			{
			   kakao.maps.event.addListener(map, "click", function(mouseEvent)
			   {
				...
				<div>
					<span class="red">※</span> 방문예정 지역<br> <input type="text"
						class="form-control-lg" style="width: 500px;" id="local"
						name="mmRestLocation" readonly="readonly"
						value="${roomInfo.restLocation }" required>
				</div>
				<div>
					<span class="red">※</span> 방문예정 식당<br> <input type="text"
						class="form-control-lg" style="width: 500px;" id="restaurant"
						name="mmRestName" readonly="readonly"
						value="${roomInfo.restName }" required>
				</div>
				<br>
				<div>
					<span class="red">※</span> 음식종류<br> <select
						class="form-select-lg" style="width: 500px;" id="foodCategory"
						name="mmFoodCtgCode" required>
						<c:forEach var="foodCtgList" items="${foodCtgList }">
							<option value="${foodCtgList.FOOD_CTG_CODE}"
								${roomInfo.foodCtg == foodCtgList.FOOD_CASE ? "selected='selected'" : ""}>${foodCtgList.FOOD_CASE}</option>
						</c:forEach>
					</select>
				</div>
				<div>
					<span class="red">※</span> 메인메뉴명<br> <input type="text"
						class="form-control-lg" style="width: 500px;" id="mainMenu"
						name="mmMenu" placeholder="내용을 입력해주세요"
						value="${roomInfo.mainMenu }" required>
				</div>
				<div>
					<span class="red">※</span> 메인메뉴가격<br> <input type="text"
						class="form-control-lg" style="width: 500px;"
						onkeyup="perPriceCal()" id="mainMenuPrice" name="mmMenuPrice"
						value="${roomInfo.menuPrice }" placeholder="내용을 입력해주세요">(원)
				</div>
				<div>
					<span class="red">※</span> 모집 인원수<br> <input type="text"
						class="form-control-lg" style="width: 500px;"
						onkeyup="perPriceCal()" id="peopleCount" name="mmNop"
						placeholder="내용을 입력해주세요" value="${roomInfo.nopHope }">(명) &nbsp; 현재인원 : ${roomInfo.nopCurrent }
				</div>
				<div>
					<span class="blue">※</span> 인당 예상 가격<br> <input type="text"
						class="form-control-lg" style="width: 500px;" readonly="readonly"
						id="perPrice" value="${roomInfo.personalPrice }" placeholder="(원)">
				</div>
				<br>
				<div>
					<span class="red">※</span> 모집 희망 성별<br>

					<c:choose>
						<c:when test="${roomInfo.genderCtg == '무관' }">
						<select class="form-select-lg" style="width: 500px;" id="gender" name="mmGenderCode" required>
						<c:forEach items="${genderList }" var="genderList">
						<c:if test="${genderList.GENDER_CTG == '무관' }">
						...
						<c:otherwise>
							...
							<select class="form-select-lg" style="width: 500px;" id="gender" name="mmGenderCode" required>
								<c:forEach items="${genderList }" var="genderList">
									<option value="${genderList.GENDER_CODE }"
										${genderList.GENDER_CTG == roomInfo.genderCtg ? "selected='selected'": "" }>${genderList.GENDER_CTG }</option>
								</c:forEach>
							</select>
				...
				<div>
					<span class="red">※</span> 모집 희망 연령대<br> <select
						class="form-select-lg" style="width: 500px;" id="AgeGroup"
						name="mmAgeGroupCode" required>
						<c:forEach var="ageGroupList" items="${ageGroupList }">
							<option value="${ageGroupList.AGE_GROUP_CODE}"
								${ageGroupList.AGE_GROUP_CASE == roomInfo.ageGroup ? "selected='selected'" : "" }>${ageGroupList.AGE_GROUP_CASE}</option>
						</c:forEach>
					</select>
				</div>
				<!--개설키워드  -->
				<div class="form-group">
					<span class="red">※</span> 개설 키워드(최대 3개) :<input type="text"
						id="tag" class="form-control-lg item" style="width: 500px;"
						placeholder="엔터로 개설키워드를 등록해주세요." />
				</div>
				<div class="form-group">
				...
				<input type="hidden" name="mmUserCode" value="${sessionScope.userCode}"> 
				<input type="hidden" name="openCode" value="<%=openCode%>">
				<input style="display: none;" type="text" name="mmOpenKeyWord" id="keyWord">
					 <span id="Check" class="red" style="text-align: center; display: none;">※항목은
					필수입력값입니다.</span>
			</div>
			<!--end content  -->
		</div>
		...
		<div class="button_box">
			<button type="submit" class="btn btn-warning"
				style="width: 400px; font-weight: bold; font-size: x-large;">수정하기!</button>
		</div>
